...
//
// Code made by CS1521 staff
// Code Edited by Kevin Edbert Sutandi (z5352065)
// Date 15/03/2022
...
int is_compile_needed(char *c_file, char *binary) {
    struct stat stat_c;
    if (stat(c_file, &stat_c) != 0) {
        perror(c_file);
        exit(EXIT_FAILURE);
    }

    struct stat stat_binary;
    if (stat(binary, &stat_binary) != 0) {
        return 1;
    }

    if (stat_c.st_mtime > stat_binary.st_mtime) {
        return 1;
    } else{
        return 0;
    }
