...
# x in $t0, y in $t1
# MAX ARRAY IS array[42][24]
# x will be multiplied by 42 to find the correct row
# add y to find the right column
# print the value of the array at that index
#
...
main:
    ...
    mul $t2, $t0, 24    # Find row index
    add $t2, $t2, $t1   # Finding column index
    mul $t2, $t2, 4     # Multiply by 4 to get offset
    la  $t3, array      # Load array to $t3
    add $t3, $t3, $t2   # Add offset to array address

    lw $a0, 0($t3)      # Load value at offset
    li $v0, 1
    syscall             # printf("%d\n", array[x][y]);
