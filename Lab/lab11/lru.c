...
// First made by CS1521 Team
// Finished by Kevin Sutandi (z5352065)
...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    for (int counter = 0; counter < n_physical_pages; counter++) {
        if (ipt[counter].virtual_page == virtual_page) {
            placeholder = counter;
            break;
        } else if (ipt[counter].last_access_time < ipt[placeholder].last_access_time) {
            placeholder = counter;
        }
    }

    printf("Time %d: virtual page %d ", access_time, virtual_page);
    if (ipt[placeholder].virtual_page == virtual_page) {
        ipt[placeholder].last_access_time = access_time;
        printf("->"); // Just prints arrow
    } else if (ipt[placeholder].virtual_page == -1) {
        ipt[placeholder].virtual_page = virtual_page;
        ipt[placeholder].last_access_time = access_time;
        printf("loaded to");
    } else {
        int temp = ipt[placeholder].virtual_page;
        ipt[placeholder].virtual_page = virtual_page;
        ipt[placeholder].last_access_time = access_time;
        printf(" - virtual page %d evicted - loaded to", temp);
    }
